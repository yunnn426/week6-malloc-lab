...
static char *lastp; // save previous search point for next fit

/* explicit */
static char *free_listp; // points free list pred
...
static void *coalesce(void *bp) {
    ...
    // case 2: next is free
        ...
        /* explicit */
        putFreeBlock(bp); // curr block is added to free list
    }
    ...
    // case 4: both free
        ...
        /* explicit */
        removeBlock(GET(PREV_BLKP(bp)));
        removeBlock(GET(NEXT_BLKP(bp)));
...
{   
    ...
    PUT(heap_listp + (4 * WSIZE), PACK(DSIZE, 1)); // prologue footer (4byte)
    PUT(heap_listp + (5 * WSIZE), PACK(0, 1)); // epilogue header (0byte)

    /* explicit */
    free_listp = heap_listp + (2 * WSIZE); // free_listp points pred
...
static void *find_fit(size_t asize) {
    ...
    for (bp = free_listp; GET_SIZE((HDRP(bp) > 0)) > 0; bp = SUCC(bp)) {
